...
<table class="table table-bordered">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{$key->name}}</td>
      <td><img style="width:80px" src="{{asset($key->allimg)}}"></td>
      <td>{{$key->created_at}}</td>
      <td>
        <!-- Button trigger modal -->
        <a href="{{route('updateproduct', $key->id)}}" class="btn-sm btn-success">Edit</a><br><br>
     ...
     <div class="modal-dialog" role="document">
       <div class="modal-content">
         ...
         <div class="modal-footer">
           ...
           <a href="{{route('ProductDelete', $key->id)}}" class="btn btn-danger">Delete</a>
         </div>
       </div>
     </div>
   </div>
